...
class ResponseField(FloatLayout):
    ...
    def view_model(self, peek_button=False):
        if self.ids['peek_widget'].visible:
            ...
            # Have to put this here (instead of just writing not disabled) -- not too sure why
        else:
    ...
    def switch_mode(self):
        ...
        if not self.data_submitted:  # Only allow the user to switch modes if they haven't already submitted data
            ...
            if self.writing:
                ...
                self.ids['clear_button'].disabled = True
                # Prevent the user from accidentally clearing what they have drawn
            else:
                kw.width = 0
                kw.opacity = 0
    ...
    def retrieve_data(self):
        ...
        details)
        """
        # if not self.data_submitted:  # Check if data has already been submitted - prevents wasteful resubmissions
    ...
    def check_answer(self, text: str = None, confidence: float = 1):
        ...
        processed).  If the solution is incorrect, check the confidence level. If the confidence level is below 75%
...
class NavigationPane(BaseNavigationPane):
    ...
    def update_question(self):
        ...
        self.ids[str(screen.question_number)].on = True  # Update progress bar
